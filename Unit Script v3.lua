--- script by SlappyMoose, for his "D6 Kriegsspiel" system, v1.1
--- copy paste all this code into the table's Global LUA, then tag units with "D6KS Infantry" / Cavalry / Artillery
debug_global, debug_unitInit, debug_unitState = false, false, false
FORCE_GLOBAL_LOAD, SCRIPT_VERSION = false, 12
LOCAL_ASSETS = false
UNIT_LUA = [[
UNIT_VERSION = 12
...
local_assets = Global.getVar("LOCAL_ASSETS")
...
function onload(saved)
	if saved ~= "" and saved ~= nil then
		if debug_unitInit then print("onLoad data found...") end
		local data = JSON.decode(saved)
		local_data = data.saved_data or {}
	else
		if debug_unitInit then print("No onLoad data found.") end
		local_data = ""		
	end
...
function loadData()
	...
	if global_data ~= "" and global_data ~= nil or force_global_load then
	...
	if myData ~= nil and myData ~= "" then
		faction = myData.faction
		print("faction loaded from myData = ", faction)
...
function loadDefaults()
	...
	meshURLsInf = {
		full = {
			cohesive = {
			"inf_Section_Full.obj",
			"inf_Section_Full1a.obj",
			"inf_Section_Full1b.obj",
			"inf_Section_Full2a.obj",
			"inf_Section_Full2b.obj"
			},
			broken = {
			"inf_Section_Full-Br.obj",
			"inf_Section_Full1a-Br.obj",
			"inf_Section_Full2b-Br.obj"
			},
			shattered = {
			"inf_Section_Full-Sh.obj"
			}
		},
		reduced = {
			cohesive = {
			"inf_Section_Reduced.obj",
			"inf_Section_Reduced1a.obj",
			"inf_Section_Reduced1b.obj",
			},
			broken = {
			"inf_Section_Reduced-Br.obj",
			"inf_Section_Reduced1b-Br.obj"
			},
			shattered = {
			"inf_Section_Reduced-Sh.obj",
			"inf_Section_Reduced1b-Sh.obj"
			}
		},
		depleted = {
			cohesive = {
			"inf_Section_Reduced2a.obj",
			"inf_Section_Reduced2b.obj",
			"inf_Section_Reduced3a.obj"
			},
			broken = {
			"inf_Section_Reduced2a-Br.obj",
			"inf_Section_Reduced3a-Br.obj"
			},
			shattered = {
			"inf_Section_Reduced2a-Sh.obj",
			"inf_Section_Reduced3a-Sh.obj"
			}
		}
	}
	meshURLsCav = {
		full = {
			cohesive = {
			"cav_squad_full-co-0a.obj",
			"cav_squad_full-co-1a.obj",
			"cav_squad_full-co-1b.obj"
			},
			broken = {
			"cav_squad_full-br-0a.obj",
			"cav_squad_full-br-0b.obj",
			"cav_squad_full-br-1a.obj"
			},
			shattered = {
			"cav_squad_full-sh-0a.obj",
			"cav_squad_full-sh-0b.obj",
			"cav_squad_full-sh-1a.obj",
			"cav_squad_full-sh-1b.obj"
			}
		},
		reduced = {
			cohesive = {
			"cav_squad_loss-co-0a.obj",
			"cav_squad_loss-co-1a.obj",
			"cav_squad_loss-co-1b.obj"
			},
			broken = {
			"cav_squad_loss-br-0a.obj",
			"cav_squad_loss-br-0b.obj",
			"cav_squad_loss-br-1a.obj",
			"cav_squad_loss-br-1b.obj"
			},
			shattered = {
			"cav_squad_loss-sh-0a.obj",
			"cav_squad_loss-sh-0b.obj",
			"cav_squad_loss-sh-1a.obj",
			"cav_squad_loss-sh-1b.obj"
			}
		},
		depleted = {
			cohesive = {
			"cav_squad_loss-co-1a.obj",
			"cav_squad_loss-co-2a.obj",
			"cav_squad_loss-co-2b.obj"
			},
			broken = {
			"cav_squad_loss-br-1a.obj",
			"cav_squad_loss-br-1b.obj",
			"cav_squad_loss-br-2a.obj",
			"cav_squad_loss-br-2b.obj"
			},
			shattered = {
			"cav_squad_loss-sh-1a.obj",
			"cav_squad_loss-sh-1b.obj",
			"cav_squad_loss-sh-2a.obj"
			}
		}
	}
	meshURLsArt = {
	...
	local spawnFaction = Global.getVar("spawnFaction")
	print("spawnFaction = ",spawnFaction)
	faction = spawnFaction or "red"
...
posFatigue = {28, 0.26, 15}
local posVeterancy = {22, 0.26, 10}
local nameScale = 25
local fontSize = 100
local fontSize2 = 130
...
if createVeterancy then
	self.createButton({click_function = "nullFunction", function_owner = self, label = veterancyLabels[2], position = posVeterancy, rotation = {0, 0, 0}, scale = {40, 1, 40}, width = 40, height = 40, font_size = 80, color = colors.none, font_color = colors.byte.white, tooltip = "Veterancy"})
end
...
function writeUnitData()
	...
	if faction == "red" then
		...
		"img_inf-red.png",
		"img_cavalry-red.png",
		"img_artillery-red.png",
		"img_inf-red.png",
		"img_rider-red.png",
		type = {
			"img_cav-hussar-red.png",
			"img_cav-dragoon-red.png",
			"img_cav-lancer-red.png",
			"img_cav-cuirassier-red.png"
			}
		}
	elseif faction == "blue" then
		factionURLs = {
		"img_inf-blue.png",
		"img_cavalry-blue.png",
		"img_artillery-blue.png",
		"img_inf-blue.png",
		"img_rider-blue.png",
		type = {
			"img_cav-hussar-blue.png",
			"img_cav-dragoon-blue.png",
			"img_cav-lancer-blue.png",
			"img_cav-cuirassier-blue.png"
			}
		}
	else
		print("No faction assigned!")
	end
	imgURL = imgFolder .. factionURLs[unitScript]
	img2URL = imgFolder .. factionURLs.type[unitType]

	blockScale = Vector(1.45, 1, 1.45)
	if unitScript == 1 then --- INFANTRY BATTALION
		flagURL = objFolder .. "inf_Flag.obj"
		blockCount = 8
		meshURLs = objFolder .. meshURLsInf
	elseif unitScript == 2 then --- CAVALRY SQUADRON
		flagURL = objFolder .. "cav_flag.obj"
		blockCount = 3
		unitTypeCount = 4
		meshURLs = objFolder .. meshURLsCav
	elseif unitScript == 3 then --- ARTILLERY SECTION
		blockCount = 1
		meshURLs = objFolder .. meshURLsArt
	elseif unitScript == 4 then --- SKIRMISHER ZUG
		blockCount = 1
		meshURLs = objFolder .. meshUrlsSkirm
	end
...
function formLine()
	direction = "moveup3"
	if formationVal == 1 or formationVal == 4 then
...
function setFatigue()
	...
	local posFatigueSet = { posFatigue[1] + fatigueBonus * 2, posFatigue[2], posFatigue[3] }
	
	btnIndex = btnIndex+1
	self.editButton({index = btnIndex, label = fatigueLabel, position = posFatigueSet, scale = statScaleNormal, font_color = fatigueFont, tooltip = fatigueTooltip})
end
...
function setVeterancy()
	...
	self.editButton({index = btnIndex, label = veterancyLabels[veterancy]})
	
	setFatigue()
end

function getSpawnPos(offsetX, offsetY)
	if offset == nil then offset = 0 end
	local pos = self.getPosition()
	local forward = self.getTransformForward()
	local right = self.getTransformRight()
	local spawnPos = pos - (forward * offsetY) + (right * offsetX)
	return spawnPos
end
...
function spawnSmoke() --spawns smoke token
	...
	if unitScript ~=3 then
		spawnPos = getSpawnPos(0,0.6)
		imgSmoke = assetURLs.imgSmokeInf
	else
		spawnPos = getSpawnPos(0,0.9)
		imgSmoke = assetURLs.imgSmokeArt
	end
	local myRot = myRot()
	local rightDir = self.getTransformRight()
	
	local offset = 0.85
	local posLeft = spawnPos - (rightDir * offset)
	local posRight = spawnPos + (rightDir * offset)
	
	local spawnPoints = {spawnPos}
	if unitScript == 1 then
		spawnPoints = {posRight, posLeft}
	end
...
function onSave()
	return JSON.encode({ unitData = unitData })
end
