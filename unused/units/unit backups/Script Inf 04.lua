...
formSquare = {index = 4, click_function = "nullFunction", function_owner = self, label = "□", position = {0, 0.25, -0.1}, rotation = {0, 0, 0}, scale = {11, 11, 11}, width = 0, height = 0, font_size = 250, font_color = {1,1,1,127}, color = {0,0,0,0}}

formMarchL = {index = 4, click_function = "nullFunction", function_owner = self, label = "◄            ◄◄◄", position = {1.5, 0.125, 0},	rotation = {0, 0, 0}, scale = {5, 5, 5}, width = 0, height = 0, font_size = 160, font_color = {1,1,1,1}, color = {0,0,0,1}}
formMarchR = {index = 4, click_function = "nullFunction", function_owner = self, label = "◄            ◄◄◄", position = {-1.5, 0.125, 0}, rotation = {0, 180, 0}, scale = {5, 5, 5}, width = 0, height = 0, font_size = 160, font_color = {1,1,1,1}, color = {0,0,0,1}}

formAssaultL = {index = 4, click_function = "nullFunction", function_owner = self, label = "↑A↑", position = {-3.4, 0.125, 0}, rotation = {0, -90, 0}, scale = {3, 3.5, 3}, width = 0, height = 0, font_size = 200, font_color = {1,0,0,1}, color = {0,0,0,1}}
formAssaultR = {index = 4, click_function = "nullFunction", function_owner = self, label = "↑A↑", position = {3.4, 0.125, 0}, rotation = {0, 90, 0}, scale = {3.5, 3.5, 3.5}, width = 0, height = 0, font_size = 200, font_color = {1,0,0,1}, color = {0,0,0,1}}
...
self.createInput({value = name, input_function = "editName", label = "Unit", function_owner = self,
    alignment = 3, position = {0,0,-0.7}, rotation = {0, 0, 180}, width = 400, height = 110,
    font_size = 70, scale={x=1.5, y=1.5, z=1.5}, font_color= {1,1,1,1}, color = {0.1,0.1,0.1,1}})
...
----------------------------------------------------------------
self.createButton({index = 5, click_function = "clickLine", label = "Line",
	function_owner = self, label = "L", position = {0.65, 0, -0.05}, rotation = {0, 0, 180}, scale = {1.8, 1.8, 1.8}, width = 100, height = 80, font_size = 70, color = btnLine, font_color = {1, 1, 1, 1},
	tooltip = "Form Line"})

self.createButton({index = 6, click_function = "clickSquare", label = "Square",
    function_owner = self, label = "S", position = {0.65, 0, 0.30}, rotation = {0, 0, 180}, scale = {1.8, 1.8, 1.8}, width = 100, height = 80, font_size = 70, color = btnSquare, font_color = {1, 1, 1, 1},
    tooltip = "Form Square"})

self.createButton({index = 7, click_function = "clickColumn", label = "Column",
    function_owner = self, label = "C", position = {-0.65, 0, -0.05}, rotation = {0, 0, 180}, scale = {1.8, 1.8, 1.8}, width = 100, height = 80, font_size = 70, color = btnColumn, font_color = {1, 1, 1, 1},
    tooltip = "Marching Column"})

self.createButton({index = 8, click_function = "clickAssault", label = "Assault",
    function_owner = self, label = "A", position = {-0.65, 0, 0.30}, rotation = {0, 0, 180}, scale = {1.8, 1.8, 1.8}, width = 100, height = 80, font_size = 70, color = btnAssault, font_color = {1, 1, 1, 1},
...
-----------------------------------------
-------- Formation overlay stuff --------
-----------------------------------------

faction = red
imageFormation = nil
if faction == red then
	urlLine = "https://steamusercontent-a.akamaihd.net/ugc/12917511966946370192/7F18BCA3533DE1942E80B5AF5036B46AE1D085CE/"
	urlColumn = "https://steamusercontent-a.akamaihd.net/ugc/12723716944005373788/DBEDBAE24A697080DE197EA63A659108D0953BFB/"
end

function UpdateFormationOverlay()
	-- print("Starting UpdateFormationOverlay...")
	-- printDebug()
    -- if FormationOverlay ~= nil then
        -- print("Overlay already exists on this tile.")
        -- return
    -- end
	...
	local params = {
		type = "Custom_Token",
		position = self.positionToWorld(Vector(0, 0.13, 0)), -- float just above tile
		rotation = self.getRotation() + Vector(0, 0, 0),   -- flat, face-up
		scale = self.getScale() + Vector(0.22, 0, 0.22)
	}
	FormationOverlay = spawnObject(params)
	local data = {
	...
	FormationOverlay.setCustomObject(data)
	self.addAttachment(FormationOverlay)

	-- print("Finished UpdateFormationOverlay.")
	-- printDebug()
end

function RemoveFormationOverlay()
...
function printDebug()
	print("currentForm = "..currentForm)
	print("FormationOverlay = " .. (FormationOverlay and 'true' or 'false'))
	print("----------")
end

function updateForm()
	-- print("Starting updateForm...")
	-- printDebug()
	--formation + smoke check
	if currentForm == 1 then
		imageFormation = urlLine
		self.editButton(formLine)
		self.editButton({index = 5, label = "F", font_color = 'Yellow', tooltip = "Fire!"})
	else
		imageFormation = urlColumn
	...
	if FormationOverlay ~= nil then else UpdateFormationOverlay() end
...
function clickColumn()
	if currentForm == 3 then changeDirection() end
	-- if currentForm == 1 then resizeColumn() end
...
function clickAssault()
	if currentForm == 4 then changeDirection() end
	-- if currentForm == 1 then resizeColumn() end
